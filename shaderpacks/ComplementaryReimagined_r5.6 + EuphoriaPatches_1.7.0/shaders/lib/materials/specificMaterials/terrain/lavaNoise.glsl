    ...
    if (mat == 10070) { // Flowing Lava
        ...
        lavaNoiseColor += min(pow2(pow2(lavaNoiseEmission * 0.50)), 0.2) * LAVA_TEMPERATURE * 0.65 + 0.1;
        #ifdef NETHER
            if ((worldPos.y > 30 && worldPos.y < 32.3 || worldPos.y > 35 && worldPos.y < 37.3) && BLOCK_LAVA_STILL_DEFINE) { // lava lakes in the nether
                ...
                noise += texture2DLod(noisetex, lavaPos * 0.05 + wind * 0.01, 0.0).r;
                noise -= texture2DLod(noisetex, lavaPos * 1.5 + wind * 0.05, 0.0).r * 0.3;
            ...
            if (worldPos.y > -56 && worldPos.y < -53.7 && BLOCK_LAVA_STILL_DEFINE) { // lava lakes in the Overworld, End not affected because no negative coords
                ...
                noise += texture2DLod(noisetex, lavaPos * 0.2 + wind * 0.01, 0.0).g;

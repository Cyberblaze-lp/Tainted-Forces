...
vec3 ApplyMultiColoredBlocklight(vec3 blocklightCol, vec3 screenPos, vec3 playerPos, float lmCoord) {
    ...
    float entityMask = step(0.5, sqrt3(coloredLight.a)) * lmCoordStep;
        ...
        if (ACTDecider < 0.5 && entityMask < 0.5) return blocklightCol;
    #endif
    ...
    return mix(blocklightCol, coloredLightNormalized, coloredLightMix * clamp01(MCBL_INFLUENCE) * (1.0 - float(isnan(coloredLightNormalized))) * lmCoordStep);
}